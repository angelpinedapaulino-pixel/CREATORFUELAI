              ...
              <div className="space-y-4">
                {tool.inputs.map((inp) => (
                  <div key={inp.key} className="flex flex-col gap-1.5">
                    ...
                    {inp.type === 'select' ? (
                      <Select value={form[inp.key] ?? ''} onValueChange={(v) => setField(inp.key, v)}>
                      ...
                      <Textarea id={inp.key} rows={3} placeholder={inp.placeholder} value={form[inp.key] ?? ''} onChange={(e) => setField(inp.key, e.target.value)} />
                    ) : (
                      <Input id={inp.key} placeholder={inp.placeholder} value={form[inp.key] ?? ''} onChange={(e) => setField(inp.key, e.target.value)} />
                    )}
      ...
      <Dialog open={saveOpen} onOpenChange={setSaveOpen}>
        <DialogContent>
          ...
          <div className="flex flex-col gap-1.5 py-2">
            ...
            <Input id="save-title" value={saveTitle} onChange={(e) => setSaveTitle(e.target.value)} maxLength={120} />
          </div>
          <DialogFooter>
